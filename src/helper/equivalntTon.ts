...
export async function getEquivalentTon(value:string) {
    try {
        //@ts-ignore
        ...
        // Convert TON to its smallest unit (wTON has 9 decimals)
            const bmbtcAmount = parseFloat(value)*10**9;
            
            // Call the contract function
            const balance: bigint = await contract.calculateBmbtcAmount(bmbtcAmount);
            console.log('oracle value : ', balance)

            // Convert BigInt to readable format (8 decimals)
            const formattedBalance = Number(ethers.formatUnits(balance.toString(), 9));
